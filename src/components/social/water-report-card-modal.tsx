...
import {
  X, Download, Copy, Share2, Check, ExternalLink,
  Sparkles, ShieldAlert, Droplets,
} from 'lucide-react'
...
export function WaterReportCardModal({
  ...
  useEffect(() => {
    ...
    // Standard high-resolution landscape card for Twitter / LinkedIn (1200 x 630)
    const W = 1200
    const H = 630
    canvas.width = W
    canvas.height = H
    ...
    const popText = utility.populationServed
      ? ` · ${utility.populationServed.toLocaleString()} residents served`
      : ''
    ...
    ctx.fillStyle = '#ffffff'
    ctx.fillText(`${utility.contaminantSummaries.length}`, 95, cardY + 92)
    ...
    ctx.fillText(`${utility.healthExceedances}`, card2X + 25, cardY + 92)

    // Stat 3: Legal Exceedances or Microplastics
    const card3X = card2X + cardW + gap
    const hasLegalExceedance = utility.exceedances > 0
    ...
    ctx.fillText(hasLegalExceedance ? `${utility.exceedances} Violations` : 'Within Legal Limits', card3X + 25, cardY + 92)
    ...
    const topItems = utility.contaminantSummaries.slice(0, 3)
    topItems.forEach((item, idx) => {
      ...
      ctx.fillText(item.contaminant.name, 108, y + 20)

      // Value & Unit
      ctx.font = 'mono 14px monospace'
      ctx.fillStyle = '#94a3b8'
      ctx.fillText(`${item.latestLevel} ${item.unit}`, W - 320, y + 20)
  ...
  const handleShareX = () => {
    const exceedanceStr = utility.healthExceedances > 0
      ? `${utility.healthExceedances} contaminants exceed EWG health guidelines`
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-border/60 px-5 py-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                High-resolution 1200×630 graphic ready to post, paste into Discord/Slack, or save.
              </p>
            </div>
            <Button
              variant="ghost"
          ...
          <div className="p-5 bg-muted/20">
            <div className="overflow-hidden rounded-xl border border-border/80 shadow-lg">
