...
export async function GET() {
  const [
    ...
    db.sample.count(),
    db.report.count(),
    db.utility.findMany({ select: { state: true, population: true } }),
    db.sample.findMany({
      select: {
        level: true,
  ...
  // Exceedance counts across all samples (latest per contaminant-utility pair)
  let healthExceedances = 0
  let legalExceedances = 0
  for (const s of samples) {
    const hg = s.contaminant.healthGuideline
    const ll = s.contaminant.legalLimit
    if (hg != null && hg > 0 && s.level > hg) healthExceedances++
    if (ll != null && ll > 0 && s.level > ll) legalExceedances++
  }
